<div class="overlay overlay-landscape">
	<div class="fadeout">
		<div class="container">
			<div class="col-md-6 p-lg-5 mx-auto my-5">
				<h1>Legend's <span class="text-warning">Builders</span></h1>
				<p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Labore ipsam beatae, sequi ipsa nulla necessitatibus.</p>
			</div>
		</div>
	</div>
</div>

<main class="bg-parchment" role="main">
    <div class="container">
        <div class="row">
            <div class="col-md-4">
                <div class="card card-parchment mb-4">
                    <div class="card-body">
                        <div class="card-block">
                            <h5 class="card-title">Sibel Urbain</h5>
                            <p class="card-text"><span class="text-expose">UI Designer</span> & <span class="text-expose">Game Writer</span></p>
                        </div>
                        <div class="card-block card-footer">
                            <div class="row justify-content-center mt-4">
                                <div class="col-sm-5">
                                    <a href="https://github.com/UrbainSibel" target="_blank" class="btn btn-sm btn-dark btn-block"><i class="fab fa-github mr-2"></i>GitHub</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card card-parchment mb-4">
                    <div class="card-body">
                        <div class="card-block">
                            <h5 class="card-title">Axel Guihard</h5>
                            <p class="card-text"><span class="text-expose">UI Designer</span></p>
                        </div>
                        <div class="card-block card-footer">
                            <div class="row justify-content-center mt-4">
                                <div class="col-sm-5">
                                    <a href="https://github.com/AxelGhd" target="_blank" class="btn btn-sm btn-dark btn-block"><i class="fab fa-github mr-2"></i>GitHub</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-md-4">
                <div class="card card-parchment mb-4">
                    <div class="card-body">
                        <div class="card-block">
                            <h5 class="card-title">Simon Malpel</h5>
                            <p class="card-text"><span class="text-expose">Backend</span> & <span class="text-expose">Ops</span></p>
                        </div>
                        <div class="card-block card-footer">
                            <div class="row justify-content-center mt-4">
                                <di class="col-sm-5">
                                    <a href="https://twitter.com/simonmalpel" target="_blank" class="btn btn-sm btn-primary btn-block"><i class="fab fa-twitter mr-2"></i>Twitter</a>
                                </di>
                                <div class="col-sm-5">
                                    <a href="https://github.com/SimonRTC" target="_blank" class="btn btn-sm btn-dark btn-block"><i class="fab fa-github mr-2"></i>GitHub</a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="jumbotron jumbotron-wood mt-4">
            <h2 class="space-card-title mt-n5 mb-4">Remerciements</h2>
            <div class="row">
                <div class="col-md-4 mb-5">
                    <div class="text-center">
                        <h5><i class="far fa-user mr-3"></i>Loic Godin</h5>
                        <p class="badge badge-light">HTML & CSS & JS</p>
                    </div>
                </div>
                <div class="col-md-4 mb-5">
                    <div class="text-center">
                        <h5><i class="far fa-user mr-3"></i>Stéphane Menut</h5>
                        <p class="badge badge-light">jQuery & Bootstrap</p>
                    </div>
                </div>
                <div class="col-md-4 mb-5">
                    <div class="text-center">
                        <h5><i class="far fa-user mr-3"></i>Matthieu Lamamra</h5>
                        <p class="badge badge-light">Git & Ops</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="text-center">
                        <h5><i class="far fa-user mr-3"></i>Pierre Brétéché</h5>
                        <p class="badge badge-light">PHP</p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="text-center">
                        <h5><i class="far fa-building mr-3"></i>Dawan</h5>
                        <p class="badge badge-light">Soutien logistique</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</main>